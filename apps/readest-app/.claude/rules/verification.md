...
7. `cargo fmt --check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` in `apps/readest.koplugin/native/localsend-ffi` — the koplugin's standalone LocalSend FFI crate (only when `apps/readest.koplugin/native/` files changed); also run in the CI `koplugin_rust_lint` job
