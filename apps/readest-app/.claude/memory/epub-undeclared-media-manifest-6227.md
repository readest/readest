...
**Status:** readest PR https://github.com/readest/readest/pull/6245 (branch `fix/epub-undeclared-media-6227`, off origin/main) + foliate PR https://github.com/readest/foliate-js/pull/97 (branch `fix/undeclared-media-manifest`, commit `3d02e98`), both OPEN. `pnpm test` 11114 passed, `pnpm lint` and `pnpm format:check` green. **The submodule is pinned to the branch commit `3d02e98`, so it MUST be re-pinned to the squashed main commit before #6245 lands** — see [[epub-embedded-video-kotobee-1812]] for the "always check `git merge-base --is-ancestor <pin> github/main` after a foliate PR merges" trap.
