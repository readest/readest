...
**Setup:** app must be running → `adb shell cat /proc/net/unix | grep webview_devtools_remote_<pid>` → `adb forward tcp:9222 localabstract:webview_devtools_remote_<PID>`. Discover targets with a Node `http.get` to `/json/list` (set header `Host: localhost`), or with **`curl --noproxy '*'`**. The earlier note here blamed curl for "mishandling the WebView's HTTP framing" — that was wrong: this shell exports `http_proxy=https_proxy=http://127.0.0.1:8118` (see [[git-push-socks-proxy]]), so a bare `curl http://127.0.0.1:9333/json` is proxied and comes back EMPTY with exit 0. With `--noproxy '*'` both `/json` and `/json/version` return normal JSON (verified 2026-09-06 against the 0.12.6 build on the Xiaomi). Reach for a Node helper only when you need the WebSocket anyway. Connect `ws://127.0.0.1:9222/devtools/page/<id>`, then `Runtime.enable` + `Runtime.evaluate {expression:'(async()=>{...})()', awaitPromise:true, returnByValue:true}`. Helper scripts kept in `/tmp/cdp/` (eval.mjs, disc.mjs).
...
- **A stale CDP client wedges the whole endpoint.** One hung node/WebSocket client holds the WebView's single debugger slot, and then `/json/list` returns EMPTY over curl (exit 0, no body) and every new `ws://` connect hangs with no error. It looks exactly like "CDP is broken on this build". Fix: `pkill -f <your-client>` first, then re-discover. Verified 2026-09-07 after three failed probes in a row.
- **A long build locks the screen and refreezes the page.** After a ~10 min `pnpm dev-android`, the device auto-locks, so `/json/list` comes back empty again for the reason above the fold (`visible:false`). `input keyevent KEYCODE_WAKEUP` + a swipe, then `svc power stayon true`, BEFORE probing. Revert `stayon false` when done, and read `settings get system screen_off_timeout` first if you plan to change it — there is no way to recover the original value afterwards.
...
- Real book files live in **internal** storage (`/data/user/0/com.biblophile.readest/...`), not the external `Android/data/.../files` dir (that's `forbidden path` to the fs plugin). `$APPCACHE` = `/data/user/0/com.biblophile.readest/cache` holds import temp copies (in asset scope `$APPCACHE/**/*`). `adb run-as` is denied on the release build.
