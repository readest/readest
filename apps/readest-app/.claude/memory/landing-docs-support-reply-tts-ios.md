...
description: "Support email 2026-09-06 (Kurt, iOS) exposed two Listen-docs errors; how TTS actually starts and picks voices, and the readest-landing docs edit recipe"
metadata: 
  node_type: memory
  type: project
...
Support email 2026-09-06 (Kurt Tappe, iPhone): docs said "press `T` or tap the speaker icon in the top toolbar" and "iOS uses the system voices". Both wrong for the app as shipped.
...
**Landing docs recipe** (`~/dev/readest-landing`, branch `dev`): edit `src/app/[locale]/docs/<slug>/page.mdx`, bump `lastModified` for that slug in `docs/_nav.ts`, run `pnpm generate:llms` (regenerates the TRACKED `src/app/llms-full.txt/_content.ts`), then `pnpm test && pnpm lint`. `pnpm format:check` has 7 pre-existing failures in legal/blog/sync MDX; ignore unless touched. Listen page fix = readest-landing PR #35 (branch `docs/listen-tts-start-and-voice-picker`, e02df9d) MERGED 2026-09-06; the reply to Kurt was SENT 2026-09-06.
