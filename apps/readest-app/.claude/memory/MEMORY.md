...
- [Vietnamese book read by an English voice](tts-word-body-lang-en-us-6248.md) Word's `<body lang="EN-US">` beats `<html lang="vi">` (foliate getLang = NEAREST ancestor); parseSSMLLang rescued only bare `en`; fix = base subtag; fetch-probe `/api/tts/edge` recipe inside
