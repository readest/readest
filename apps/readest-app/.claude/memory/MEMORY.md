...
- [#6216 ABS playback interrupted on self-signed HTTPS](abs-media-proxy-self-signed-6216.md) API fetch accepts invalid certs, `<audio>` does not; fix = Rust loopback media proxy (hyper, per-launch secret) for player + pairing; Xiaomi-VERIFIED; PR #6268 OPEN; nix hash bump pending
...
- [#6222 macOS traffic lights gone on library / position jumps](traffic-lights-library-orphan-hide-6222.md) VERIFIED by chrox; orphan hide timer + `NSWindow.setTitle` resets the title bar (offset 6 = AppKit default); fix = native `set_window_title` same-pass re-apply, a JS re-assert still flickers; dedupe + frame observer DEAD ENDS
