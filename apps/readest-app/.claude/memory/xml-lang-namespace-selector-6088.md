...
**Fix (PR #6090, branch `fix/xml-lang-namespace-6088`, 2 commits 65fe93288 + 96c346e93):** `prefix === 'xml'` -> XML namespace twin via
...
the always-on page-layout chunk, so `<p>` got `normal`, never the book's 3. FIXED (2nd commit of #6090): the reset moved into `getParagraphLayoutStyles`, so Use Book Layout ON -> 48px at 16px,
