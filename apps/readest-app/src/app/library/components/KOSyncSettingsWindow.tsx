...
const StyledSelect: React.FC<SelectProps> = ({ value, onChange, options, className, disabled = false }) => {
  return (
    <select
      value={value}
      onChange={onChange}
      className={clsx(
        'select select-bordered w-full h-12 text-sm',
        className,
      )}
      disabled={disabled}
    >
...
export const KOSyncSettingsWindow: React.FC = () => {
  ...
  // Obtener el nombre del SO una vez
  useEffect(() => {
  ...
  useEffect(() => {
    const handleCustomEvent = (event: CustomEvent) => {
      ...
      if (event.detail.visible) {
        ...
        setPassword('');
        setConnectionStatus('');
        // Sincronizar el slider con los ajustes actuales al abrir
        const tolerance = settings.koreaderSyncPercentageTolerance;
        setToleranceSliderValue((tolerance && tolerance > 0) ? Math.round(-Math.log10(tolerance)) : 4);
      }
    };
  ...
  }, [settings.koreaderSyncServerUrl, settings.koreaderSyncUsername, settings.koreaderSyncPercentageTolerance]);
      ...
      <div className='flex flex-col gap-4 p-2 sm:p-4'>
        {isConfigured ? (
          ...
          <>
            ...
            <div className='h-4 text-center text-sm text-error'>{connectionStatus}</div>
          </>
        )}
      </div>
    </Dialog>
