...
import { useAudiobookSyncGeneration } from './useAudiobookSyncGeneration';
...
export const useAudiobookSyncDebug = (props: {
  ...
  const {
    generateTranscriptFromAudiobook: genTranscript,
    generateSyncMapFromAttachedTranscript: genSyncFromAttached,
  } = useAudiobookSyncGeneration({ getView, appService, getConfig, setConfig });
  ...
  useEffect(() => {
    ...
    const api: CitadelAudiobookSyncDebugApi = {
      ...
      async generateSyncMapFromAttachedTranscript(): Promise<{
        ...
        return genSyncFromAttached(bookKey);
      },
      ...
      async generateTranscriptFromAudiobook(options?: {
        ...
        return genTranscript(bookKey, options);
      },
