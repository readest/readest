...
const LayoutPanel: React.FC<SettingsPanelPanelProp> = ({
	bookKey,
	onRegisterReset,
}) => {
	const _ = useTranslation();
	const { envConfig, appService } = useEnv();
	const { getView, getViewSettings, getGridInsets, setViewSettings } =
		useReaderStore();
	...
	const [paragraphMargin, setParagraphMargin] = useState(
	...
	const [compactMarginLeftPx, setCompactMarginLeftPx] = useState(
		viewSettings.compactMarginLeftPx!,
	);
	const [compactMarginRightPx, setCompactMarginRightPx] = useState(
		viewSettings.compactMarginRightPx!,
	);
	const [maxColumnCount, setMaxColumnCount] = useState(
		viewSettings.maxColumnCount!,
	);
	const [maxInlineSize, setMaxInlineSize] = useState(
	...
	const [overrideLayout, setOverrideLayout] = useState(
	...
	const [showBarsOnScroll, setShowBarsOnScroll] = useState(
		viewSettings.showBarsOnScroll!,
	);
	const [showRemainingTime, setShowRemainingTime] = useState(
		viewSettings.showRemainingTime!,
	);
	const [showRemainingPages, setShowRemainingPages] = useState(
		viewSettings.showRemainingPages!,
	);
	const [showPageNumber, setShowPageNumber] = useState(
		viewSettings.showPageNumber!,
	);
	const [screenOrientation, setScreenOrientation] = useState(
		viewSettings.screenOrientation!,
	);

	const [readingProgressStyle, setReadingProgressStyle] = useState<
		"percentage" | "fraction"
	...
	useEffect(() => {
		...
		if (showHeader && !viewSettings.vertical) {
			const minMarginTop = Math.max(
				0,
				Math.round((44 - gridInsets.top) / 4) * 4,
			);
			viewSettings.marginTopPx = Math.max(
	...
	useEffect(() => {
		...
		if (showFooter && !viewSettings.vertical) {
			const minMarginBottom = Math.max(
				0,
				Math.round((44 - gridInsets.bottom) / 4) * 4,
			);
			viewSettings.marginBottomPx = Math.max(
				viewSettings.marginBottomPx,
				minMarginBottom,
			);
			...
			{mightBeRTLBook && (
				<div className="flex items-center justify-between">
					...
					<div className="flex gap-4">
						...
						<div
							className="lg:tooltip lg:tooltip-bottom"
							data-tip={_("Horizontal Direction")}
						>
							<button
				...
				<div className="card bg-base-100 border-base-200 border shadow">
					<div className="divide-base-200 divide-y">
						<NumberInput
							...
							value={
								showHeader && !isVertical ? marginTopPx : compactMarginTopPx
							}
							onChange={
						...
						<NumberInput
							...
							value={
								showFooter && isVertical ? marginLeftPx : compactMarginLeftPx
							}
							onChange={
								showFooter && isVertical
									? setMarginLeftPx
									: setCompactMarginLeftPx
							}
							min={0}
							max={88}
							step={4}
						...
						<NumberInput
							...
							value={
								showHeader && isVertical ? marginRightPx : compactMarginRightPx
							}
							onChange={
								showHeader && isVertical
									? setMarginRightPx
									: setCompactMarginRightPx
							}
							min={0}
							max={88}
							step={4}
					...
					<div className="divide-base-200 divide-y">
						...
						<div className="config-item">
							...
							<select
								...
								value={readingProgressStyle}
								disabled={!showFooter}
								onChange={(e) =>
									setReadingProgressStyle(
										e.target.value as "percentage" | "fraction",
									)
								}
							>
								<option value="percentage">{_("Percentage")}</option>
								<option value="fraction">{_("Fraction")}</option>
							</select>
						</div>
						<div className="config-item">
