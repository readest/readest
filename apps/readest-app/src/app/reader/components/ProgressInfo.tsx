...
const ProgressInfoView: React.FC<PageInfoProps> = ({
	...
	const { readingProgressStyle } = viewSettings;
	...
	const pageLeft =
		pages - 1 > page
			? _("{{count}} pages left in chapter", { count: pages - 1 - page })
			: "";
	...
	return (
		...
		>
			...
			{viewSettings.showPageNumber && (
				<span className="ms-auto text-end">{pageInfo}</span>
			)}
		</div>
	);
};
