	...
	initViewState: async (
		...
		try {
			if (!bookData) {
				...
				const content = (await appService.loadBookContent(
					book,
					settings,
				)) as BookContent;
				...
				if (bookDoc.toc?.length && bookDoc.sections?.length) {
					...
					const sections = bookDoc.sections.reduce(
						(map: Record<string, SectionItem>, section) => {
			...
			set((state) => ({
				viewStates: {
					...
					[key]: {
						...
						progress: null,
						ribbonVisible: false,
						viewSettings: JSON.parse(
							JSON.stringify(configViewSettings),
						) as ViewSettings,
					},
				},
			}));
