import type { BookDoc, BookMetadata, SectionItem } from '@/libs/document';
import { CFI } from '@/libs/document';
import { buildChapterFootnotes, extractFootnoteDefs } from './mdFootnotes';

// Turn sanitized HTML into an in-memory foliate-js book at runtime (no EPUB
// conversion); shared by the Markdown (.md) and HTML (.html) loaders. The
// document is split into one section per top-level heading so reading
// progress, the cross-device location cursor and TTS section tracking all work
// per chapter — the same contract fb2.js gives a single-file format.
// Layout/font/theme styling applies for free because the reader styles
// whatever HTML the view renders.
...
/**
 * @param safeHtml body markup that already went through `sanitizeHtml`
...
export function buildHtmlBook(
  ...
  // Serialize each section to well-formed XHTML. The input carries HTML5 void
  // tags (<br>, <hr>, <img>) that are parse errors under application/xhtml+xml,
  // so XMLSerializer (not innerHTML) is required. Map every id to its section.
  ...
  const transformSection = async (index: number): Promise<string> => {
    ...
    try {
      ...
      // Readonly, mirroring foliate's Loader.createURL dispatch. These sections
      // carry no spine CFI, so selection-scoped proofread rules fall back to
      // matching their sectionHref against this name; both are the section
      // index ("<index>", or "<index>#<anchor>" via split('#')[0]).
  ...
  const sections: HtmlSection[] = xhtml.map((str, index) => ({
    id: String(index),
