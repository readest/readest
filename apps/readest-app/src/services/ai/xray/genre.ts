...
export function detectGenre(metadata: {
  ...
  const genreKeywords: Record<BookGenre, string[]> = {
    fantasy: ['fantasy', 'magic', 'wizard', 'dragon', 'sword', 'sorcery', 'mythical', 'realm'],
    'sci-fi': [
      'science fiction',
      'sci-fi',
      'scifi',
    ...
    mystery: ['mystery', 'detective', 'crime', 'murder', 'investigation', 'thriller', 'suspense'],
    romance: ['romance', 'love', 'relationship', 'wedding', 'heart', 'passion'],
    history: ['history', 'historical', 'war', 'century', 'ancient', 'medieval', 'victorian'],
    biography: ['biography', 'memoir', 'autobiography', 'life story'],
    'non-fiction': ['non-fiction', 'nonfiction', 'essay', 'journalism', 'documentary'],
    fiction: ['fiction', 'novel', 'story', 'literary'],
    unknown: [],
  };
...
export function getGenreHints(genre: BookGenre): GenreHints {
  const hintsMap: Record<BookGenre, GenreHints> = {
    ...
    'non-fiction': {
      ...
      extractionFocus: ['concepts', 'definitions', 'arguments', 'evidence', 'claims'],
    },
    fiction: {
      genre: 'fiction',
      hints: [
        'Focus on character development and plot progression',
        'Track themes and motifs throughout',
        'Identify turning points and conflicts',
      ],
      extractionFocus: ['characters', 'plot', 'themes', 'conflicts', 'development'],
    },
    unknown: {
      genre: 'unknown',
